...
public class ManageSiswa extends javax.swing.JPanel {
    ...
    private void loadSiswa() {
        ...
        for (com.pemkom.objects.Murid m : muridDAO.findAll()) {
            gridPanel.add(buatCard(
                    m.getUidRfid(), m.getNamaLengkap(), m.getUidRfid(), m.getSekolah()
            ));
        }
    ...
    private void searchSiswa() {
        ...
        java.util.List<com.pemkom.objects.Murid> daftarMurid;

        if (!sekolahFilter.equals("Semua")) {
            daftarMurid = muridDAO.findMany(
                    com.mongodb.client.model.Filters.eq("sekolah", sekolahFilter)
            );
        } else {
            daftarMurid = muridDAO.findAll();
        }

        for (com.pemkom.objects.Murid m : daftarMurid) {
            if (keyword.isEmpty()
                    || m.getNamaLengkap().toLowerCase().contains(keyword)
                    || m.getUidRfid().toLowerCase().contains(keyword)) {
                gridPanel.add(buatCard(
                        m.getUidRfid(), m.getNamaLengkap(), m.getUidRfid(), m.getSekolah()
                ));
            }
    ...
    private javax.swing.JPanel buatCard(String id, String nama, String uid, String sekolah) {
        ...
        btnDelete.addActionListener(e -> {
            ...
            if (konfirmasi == javax.swing.JOptionPane.YES_OPTION) {
                com.pemkom.objects.GenericDAO<com.pemkom.objects.Murid> muridDAO
                        = new com.pemkom.objects.GenericDAO<>("Murid", com.pemkom.objects.Murid.class);
                muridDAO.delete(com.mongodb.client.model.Filters.eq("uidRfid", uid));
                loadSiswa();
            }
        });
